...
class BatchDownloadTest {
    ...
    void generateLinks1Digits() {
        ...
        for (int i = 0, j = 0; i < end; i++, j++)
            assertEquals(l.get(j).getUri(), "https://www.file.com/file%d%d%d%d.so".formatted(0, 0, 0, i + 1));
    ...
    void generateLinks2Digits() {
        ...
        for (int i = start - 1, j = 0; i < end; i++, j++) {
            if ((i + 1) / 10 > 0)
                assertEquals(l.get(j).getUri(), "https://www.file.com/file%d%d%d.so".formatted(0, 0, i + 1));
            else
                assertEquals(l.get(j).getUri(), "https://www.file.com/file%d%d%d%d.so".formatted(0, 0, 0, i + 1));
        }
    }
    ...
    void generateLinks3Digits() {
        ...
        for (int i = start - 1, j = 0; i < end; i++, j++) {
            if ((i + 1) / 100 > 0)
                assertEquals(l.get(j).getUri(), "https://www.file.com/file%d.so".formatted(i + 1));
            else
                assertEquals(l.get(j).getUri(), "https://www.file.com/file%d%d.so".formatted(0, i + 1));
        }
    }
